...
class FlatNode:
    ...
    def __init__(self, original_node: Dict[str, Any], config: MenuConfig, menu_data: MenuData):
        ...
        # Инициализация контролов на основе правил из MenuData
        self._init_controls()

    def _init_controls(self):
        """Инициализация контролов на основе роли и правил MenuData"""
        if self.type is None or self.role is None:
            return
            
        self._controls = []
        
        # Получаем правила для текущей роли
        role_rules = self._menu_data.get_role_rules(self.role)
        if not role_rules:
            return
            
        # Обрабатываем каждый разрешенный контроль
        for control_name in role_rules.get("allowed_controls", []):
            control_type = ControlType(control_name)
            control_config = self._menu_data.get_control_config(
                self.role, control_type, self.navigate
            )
    ...
    def __repr__(self):
        ...
        # Показываем доступные контролы
        controls_str = ""
        if self._controls:
            controls_str = " [" + ", ".join([c["type"].value for c in self._controls]) + "]"
        
        return f"FlatNode({tree_flag} {cyclic_flag} {self.id}{controls_str}, {self.sibling_index + 1}/{self.sibling_count})"
