...
class MenuProcessor:
    ...
    def save_flattern_json(self, output_path: str = None)->bool:
        try:
            json_file = self.config.get('output_flattern') if output_path is None else output_path
    ...
    def get_template_nodes(self)->Dict[str, Dict]|None:
        if not self._nodes:
            return None
        return {n.id: n.template_data for n in self._nodes if n.id != 'root'}

    @property
    def get_template_leafs(self)->Dict[str, Dict]|None:
        if not self._nodes:
            return None
        return {n.id: n.template_data for n in self._nodes if n.id != 'root' and n.first_child is None}
    
    @property
    def get_unique_functions(self)->Dict[str, Dict] | None:
        if not self._nodes:
            return None
        functions = {}
        for node in self._nodes:
            if node.function_cb:
                functions[node.function_cb.get("name")] = node.function_cb
        return functions
    ...
    def get_template_branches(self)->Dict[str, Dict]|None:
        if not self._nodes:
            return None
        return {n.id: n.template_data for n in self._nodes if n.id != 'root' and n.first_child is not None}
    ...
    @property
    def get_first_node_id(self)->str | None:
        for node in self._nodes:
            # Проверяем что все необходимые атрибуты существуют
            if (hasattr(node, 'prev_sibling') and 
                hasattr(node, 'parent') and 
                ...
                hasattr(node.parent, 'id') and
                node.parent.id == 'root'):
                return node.parent.first_child.id
        return None
...
def main() -> bool:
    ...
    print(processor.get_unique_functions)
